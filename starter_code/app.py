...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://postgres:wafa@localhost:5432/fyyur'
...
class Venue(db.Model):
    ...
    child_show=db.relationship('Show',backref='parent_venue',lazy=True)
...
class Artist(db.Model):
    ...
    child_show=db.relationship('Show',backref='parent_artist',lazy=True)
...
class Show(db.Model):
  ...
  def __repr__(self):
    return f'<Show Artist ID : {self.artist_id} , Venue ID : {self.artist_id} >'
...
def venues():
  ...
  data=[]
  venues=[]
  venue=Venue.query.group_by(Venue.city,Venue.state).all()
  upcoming_shows=db.session.query(Show).join(Venue).filter(Show.start_time>datetime.now()).all()
  for show in  upcoming_shows:
    data.append({ "city": show.city,
    "state": show.state })
    for x in venue:
      venues.append({ "id": x.id,
      "name": x.name,
...
def show_venue(venue_id):
  ...
  artist=db.session.query(Show).join(Artist).join(Venue).filter(venue_id).all()
  past_shows=[]
  upcoming_shows =[]
  ...
  for showed in past_show :
       past_shows.append({
      "artist_id": showed.artist.id,
      "artist_name": showed.artist.name,
      "artist_image_link":showed.artist.image_link ,
      "start_time": showed.start_time.strftime('%Y-%m-%d %H:%M:%S')
      })

  for shows in  upcoming_show :
...
def create_venue_submission():
  ...
  # TODO: modify data to be the data object returned from db insertion
   ...
   try: 
      ...
      genres=request.get_json()['geners']
      ...
      image_link=image_link,facebook_link=facebook_link,website=website,seeking_description=seeking_description)
...
def show_artist(artist_id):
  ...
  venue=db.session.query(Show).join(Artist).join(Venue).filter(artist_id).all()
  
  past_shows=[]
  upcoming_shows =[]
  upcoming_show=db.session.query(Show).join(Artist).filter(Show.artist_id==artist_id).filter(Show.start_time>datetime.now()).all()
  past_show=db.session.query(Show).join(Artist).filter(Show.artist_id==artist_id).filter(Show.start_time<datetime.now()).all()
  for showed in past_show :
       past_shows.append({
      "venue_id": showed.venue.id,
      "venue_name": showed.venue.name,
      "venue_image_link":showed.venue.image_link ,
      "start_time": showed.start_time.strftime('%Y-%m-%d %H:%M:%S')    #2019-05-21T21:30:00.000Z"
      })
  ...
  for shows in  upcoming_show :
        ...
        "start_time":shows.start_time.strftime('%Y-%m-%d %H:%M:%S')   #"2035-04-01T20:00:00.000Z" 
        })
...
def shows():
  ...
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  data=[]
  show_all=db.session.query(Show).join(Venue).join(Artist).all()
